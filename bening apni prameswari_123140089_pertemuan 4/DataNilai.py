...
def input_data_baru(data):
    ...
    try:
        ...
        print("Input nilai tidak valid! Harap masukkan angka. Data tidak tersimpan.")
    except Exception as e:
        print(f"Terjadi kesalahan: {e}")

def filter_mahasiswa_by_grade(data):
    """Menampilkan mahasiswa yang difilter berdasarkan grade."""
    if not data:
    ...
    while True:
        ...
        try:
            ...
            if pilihan == 1:
                ...
                if tertinggi:
                    print(f"\nNilai tertinggi diraih oleh: {tertinggi['nama']} ({tertinggi['NIM']})")
                    print(f"Nilai Akhir: {tertinggi['nilai_akhir']:.2f} (Grade: {tertinggi['grade']})")
            
            elif pilihan == 4:
                terendah = cari_nilai_terendah(data_mahasiswa)
                if terendah:
                    print(f"\nNilai terendah diraih oleh: {terendah['nama']} ({terendah['NIM']})")
